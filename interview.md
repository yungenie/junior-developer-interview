...
### :heavy_check_mark: **클래스, 객체, 인스턴스**
  ...
  - **객체**  
    \- 클래스로 구현할 모든 대상입니다.
  - **인스턴스**  
    \- 객체가 메모리에 올라가(할당된) 실제 사용될 때를 뜻합니다.  
...
### :heavy_check_mark: **클래스 멤버, 인스턴스 멤버, 정적 멤버**

  - **클래스 멤버**  
    \- 인스턴스 멤버와 정적 멤버로 구분하여 선언할 수 있습니다.  

  - **인스턴스 멤버**  
    \- 객체마다 가지고 있는 멤버로, 객체가 생성될 때 객체의 필드와 메소드를 가지고 있습니다. 
    
  - **정적 멤버**  
    \- 객체들이 공유하는 멤버 입니다. 
</br>     

### :heavy_check_mark: **static, final, 상수**   
  
  - **static 키워드**     
    \- static 키워드를 사용하면 객체를 생성하지 않아도 공유되는 정적 멤버입니다.  
    \- 클래스 로더로 부터 바이트코드을 읽어 메소드 메모리 영역에 적재되며, 로딩이 끝나면 바로 사용할 수 있습니다.  
    \- 그러므로, 프로그램이 시작할 때 객체를 생성하지 않아도 사용할 수 있으며, 프로그램이 종료될 때 소멸됩니다.  

    ##### [`+여기서 잠깐`]() 정적 메소드 선언 시 주의할 점!  
        - 정적 메소드 내에서 인스턴스 필드나 메소드에 접근을 할 수 없습니다.  
        - 정적 메소드 내에서 객체 생성 후 참조 접근이 가능하며, this 키워드로 사용 불가능 합니다.  
    
  - **final 키워드**    
    \- final 키워드는 한번 초기값이 저장되면 이것이 최종적인 값이 되어 프로그램 실행 도중에 수정할 수 없습니다.  
    \- final 키워드는 객체마다 저장됩니다.  

  - **상수**     
    \- 불변의 값으로, 객체를 생성하지 않아도 공유되는 변하지 않는 값입니다.  
    \- 상수 이름은 모두 대문자로 작성하는 것이 관례입니다. 서로 다른 언어가 혼합될 시 언더바(_)로 단어들을 연결해줍니다.  
</br> 
...
### :heavy_check_mark: **자바 메모리 영역(JVM 메모리 영역, Runtime Data Area)**
  ...
  - **메서드 영역**   
    ...
    \- JVM이 읽어들인 클래스, 인터페이스에 대한 정보, 필드와 메소드, 런타임 풀 상수,  Static/final 변수, 생성자(Constructor)등 저장   
  ...
  - **스택 영역**  
    \- 로컬변수, 기본 타입 변수(값)와 참조 타입 변수(주소,번지)가 저장됨.  
    \- 블록문을 벗어나면 스택에서 제거됩니다.  
...
- 이를 총칭해서 컬렉션 프레임워크라고 부릅니다.

- **List 컬렉션**  
  - 순서가 있는 객체의 집합으로 객체의 중복을 허용합니다. (객체를 인덱스로 관리, 저장용량capacity 자동 증가)  
  - ArrayList : 단방향 포인터 구조, 인덱스로 접근하므로 검색시 유리, 삽입/삭제가 발생할 경우 인덱스 조정 추가 작업이 필요함.  
  - Vector : ArrayList와 동일한 내부 구조, 동기화된 메소드로 구성되어 있음.  
...
- **Set 컬렉션**    
  ...
  - 인덱스로 관리하지 않기때문에 저장 순서가 유지되지 않습니다.  
...
### :heavy_check_mark: **컴파일에러와 런타임에러 차이**   
  - **컴파일에러**   
    \- 문법상 오류인 구문에러 (Syntax error)  
    \- 소스코드 작성 후 저장을 하면 자동으로 .class 파일이 만들어진다. 작성 도중 구문이 맞지 않으면 해당 라인 및 문자에 빨간색 줄로 오류 표시됨.  
    
   - **런타임에러**   
    \- 애플리케이션을 실행시 발생하는 에러
    \- 컴파일 후 실행하면서 발생하는 에러. NullPointException , 무한루프도는것 , 0으로 나누는 것 등  
...
- 자바 객체 지향의 특성과 설계를 원칙을 바탕으로 자바 애플리케이션 개발을 용이하게 해주는 기본 뼈대, 구조입니다. 

### :heavy_check_mark: **Spring Container 스프링 컨테이너**   
- 스프링 컨테이너는 Bean(객체)들의 생명주기를 관리하고, 필요에 따라 객체 간 의존성 주입을 해준다.

### :heavy_check_mark: **IOC(Inversion of Control)제어의 역전**  
- 스프링 컨테이너가 필요에 따라 Bean들을 관리(제어) 해줍니다.  
- 일반적인 상황에서는 new 연산자를 통해 객체를 생성하고, 의존관계를 맺어주고, 초기화 등등을 해주지만  
- 스프링에서는 xml, 어노테이션 방식으로 스프링컨테이너에 Bean(객체)를 등록하면 Bean생명주기(생성, 의존성 관계, 초기화, 소멸)을 전부 관리합니다. 
- IOC는 DI의 상위 개념입니다.
</br>

### :heavy_check_mark: **DI(Dependency Injection)의존성 주입**   
- 각 객체 간의 의존관계를 빈 설정 정보를 바탕으로 컨테이너가 매핑시켜줍니다. 
- 스프링 컨테이너가 어떤 객체에 또 다른 객체와 의존성을 맺어주는 행위입니다.   
- 필요한(의존하는) 클래스를 직접 생성하는 것이 아닌, 주입해줌으로써 객체 간의 결합도를 줄이고 좀 더 유연한 코드를 작성할 수 있게됩니다. 

  ##### **[`+여기서 잠깐`]() 스프링을 통한 의존성 주입**  
      - @Autowired type과 id 중에 type의 매칭 우선순위가 높다. (스프링 프레임워크 어노테이션)
      - @Resource type과 id 중에 id의 매칭 우선순위가 높다. (자바 표준 어노테이션)   
...
- HTTP URL을 통해 자원을 명시하고, HTTP Method(POST, GET, PUT, DELETE)를 통해 해당 자원에 대한 CRUD를 수행합니다.
    ...
    ##### [`+여기서 잠깐`]() api란?  
        - 서로 다른 프로그램에서 요청과 응당을 주고 받을 수 있도록 만든 체계입니다.
        - 특정 기술을 사용하기 유용하게 만들어줍니다. 
